...
import { userValidation, loginValidation, handleValidationErrors } from "./middleware/validation";
import { requireAuth, verifyPassword, type AuthenticatedRequest } from "./middleware/auth";
import { logError } from "./utils/logger";

export async function registerRoutes(app: Express): Promise<Server> {
  
  // Authentication Routes
  app.post('/api/auth/register', userValidation, handleValidationErrors, async (req: Request, res: Response) => {
  ...
  app.post('/api/auth/login', loginValidation, handleValidationErrors, async (req: Request, res: Response) => {
    const authReq = req as AuthenticatedRequest;
    try {
      const { username, password } = req.body;
