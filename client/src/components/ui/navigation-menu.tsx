...
>(
  (
    { className, mobileOptimized = true, ...props }: NavigationMenuContentProps,
...
>(
  (
    { className, mobileOptimized = true, ...props }: NavigationMenuLinkProps,
    ref: React.Ref<React.ElementRef<typeof NavigationMenuPrimitive.Link>>,
  ) => {
    const isMobile = useIsMobile();
    const touchHandlers = useTouch({
      provideFeedback: true,
