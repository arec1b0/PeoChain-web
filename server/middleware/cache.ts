...
staticCache.on("set", (key, value) => {
  logInfo(`Cache SET: ${key}`, { cache: 'static', size: JSON.stringify(value).length });
});
...
dynamicCache.on("set", (key, value) => {
  logInfo(`Cache SET: ${key}`, { cache: 'dynamic', size: JSON.stringify(value).length });
});
...
export function invalidateCache(pattern: string, cacheType: 'static' | 'dynamic' | 'all' = 'all') {
  const caches = cacheType === 'all' ? [staticCache, dynamicCache] : 
                 cacheType === 'static' ? [staticCache] : [dynamicCache];
...
export function conditionalCache(cacheType: 'static' | 'dynamic' = 'static', ttl?: number) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const middleware = cacheType === 'static' ? staticDataCache(ttl) : dynamicDataCache(ttl);
...
export function monitorCacheMemory() {
  ...
  if (totalKeys > 80) { // Warn if approaching limits
    logWarn('Cache approaching key limit', { totalKeys, limit: 150 });
  }
  
  if (totalSize > 50 * 1024 * 1024) { // Warn if over 50MB
    logWarn('Cache memory usage high', { totalSize, sizeMB: totalSize / 1024 / 1024 });
  }
